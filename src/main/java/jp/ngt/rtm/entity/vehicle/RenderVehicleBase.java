...
public final class RenderVehicleBase extends Render {
            ...
            for (int i = 0; i < 2; ++i)//0;通常, 1:発光
            {
                ...
                if (i == 1) {
                    ...
                    if (flag) {
                        ...
                        IntStream.range(0, lights.length).forEach(k -> {
                    ...
                    //車内灯
                    if (flag) {
                        IntStream.range(0, lights.length).forEach(RenderUtil::disableCustomLighting);
                    }
    ...
    protected void renderRollsign(EntityVehicleBase vehicle, ModelSetVehicleBaseClient modelset) {
        ...
        for (int i = 0; i < modelset.getConfig().rollsigns.length; ++i) {
            ...
            Arrays.stream(rollsign.pos, 0, modelset.getConfig().rollsigns[i].pos.length)
                    .forEach(pos -> {
                        tessellator.addVertexWithUV(pos[3][0], pos[3][1], pos[3][2], uMin, vMin);
                        tessellator.addVertexWithUV(pos[2][0], pos[2][1], pos[2][2], uMin, vMax);
                        tessellator.addVertexWithUV(pos[1][0], pos[1][1], pos[1][2], uMax, vMax);
                        tessellator.addVertexWithUV(pos[0][0], pos[0][1], pos[0][2], uMax, vMin);
                    });
            tessellator.draw();
