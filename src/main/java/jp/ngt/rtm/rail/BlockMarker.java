...
public class BlockMarker extends BlockContainer {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
		...
		if (item != null) {
			...
			if (item.getItem() == RTMItem.wrench) {
				if (!world.isRemote) {
					if (marker.startY < 0) {
						this.onMarkerActivated(world, x, y, z, player, false);
					} else {
						this.onMarkerActivated(world, marker.startX, marker.startY, marker.startZ, player, false);
					}
				}
					...
					for (int j = 0; j < hei2; ++j) {
						for (int k = 0; k < dis2; ++k) {
							...
							if (!(i == dis1 && k == dis1) && world.getBlock(x0, y0, z0) == RTMBlock.marker) {
								RailPosition rpS = this.getRailPosition(world, x, y, z, (byte) 0, (byte) 0);
								RailPosition rpE = this.getRailPosition(world, x0, y0, z0, (byte) 0, (byte) 0);
								if (rpS == null || rpE == null) {
									continue;//設置後すぐ右クリするとnullなので
								}

								if (rpE.posY >= rpS.posY) {
									return this.createRail0(world, x, y, z, x0, y0, z0, prop, makeRail, isCreative);
								} else {
									return this.createRail0(world, x0, y0, z0, x, y, z, prop, makeRail, isCreative);
								}
							}
						}
					}
	 ...
	 * y0 <= y1でなければならない
	 */
	private boolean createRail0(World world, int x0, int y0, int z0, int x1, int y1, int z1, RailProperty prop, boolean makeRail, boolean isCreative) {
		byte dir0 = this.getMarkerDir(RTMBlock.marker, world.getBlockMetadata(x0, y0, z0));
		byte dir1 = this.getMarkerDir(RTMBlock.marker, world.getBlockMetadata(x1, y1, z1));

		RailPosition rp0 = this.getRailPosition(world, x0, y0, z0, dir0, (byte) 0);
		RailPosition rp1 = this.getRailPosition(world, x1, y1, z1, dir1, (byte) 0);
		RailMap railMap = new RailMap(rp0, rp1);
		...
		if (makeRail && railMap.canPlaceRail(world, isCreative, prop)) {
			...
			TileEntityLargeRailCore tile = (TileEntityLargeRailCore) world.getTileEntity(x0, y0, z0);
	 ...
	 * @param list {x, y, z}
	 */
	private boolean createRail1(World world, int x, int y, int z, List<int[]> list, RailProperty prop, boolean makeRail, boolean isCreative) {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			int y0 = list.get(i)[1];
			int z0 = list.get(i)[2];
			byte dir = this.getMarkerDir(world.getBlock(x0, y0, z0), world.getBlockMetadata(x0, y0, z0));
			Block block = world.getBlock(x0, y0, z0);
			byte b = (byte) ((block == RTMBlock.markerSwitch) ? 1 : 0);
			RailPosition rp = this.getRailPosition(world, x0, y0, z0, dir, b);
			rpList.add(rp);
		}

		RailMaker railMaker = new RailMaker(world, rpList);
		...
		for (int[] aint : list) {
			...
			TileEntityLargeRailSwitchBase tile = (TileEntityLargeRailSwitchBase) world.getTileEntity(aint[0], aint[1], aint[2]);
	...
	private boolean createRail2(World world, int x0, int y0, int z0, RailProperty prop, boolean makeRail, boolean isCreative) {
		...
		byte dir0 = this.getMarkerDir(RTMBlock.markerSlope, meta);
	...
	private RailPosition getRailPosition(World world, int x, int y, int z, byte dir, byte type) {
		TileEntity tile = world.getTileEntity(x, y, z);
		if (tile instanceof TileEntityMarker) {
			TileEntityMarker marker = (TileEntityMarker) tile;
